    ...
    while (1) {
        ...
        if (!message.empty()) {
            if (message == "sendfile") {
                ...
                std::string command = "sendfile " + filename;
                if (send(sock, command.c_str(), BUFFSIZE, 0) == -1) {
...
int main(int argc, char* argv[]) {
    ...
    if (recv(sock, buff, BUFFSIZE, 0) == -1) std::cout << "Receive error" << std::endl;
    ...
    while (1) {
        ...
        if (msgLen == -1) std::cout << "Receive error" << std::endl;
